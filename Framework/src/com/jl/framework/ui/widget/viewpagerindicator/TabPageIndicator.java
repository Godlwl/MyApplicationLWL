 ...
 */
package com.jl.framework.ui.widget.viewpagerindicator;
...
public class TabPageIndicator extends HorizontalScrollView implements
	...
	private final OnClickListener mTabClickListener = new OnClickListener() {
		...
		public void onClick(View view) {
			// TabDotView tabView = (TabDotView) view;
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mItemCount = childCount;
		if (childCount > 1
				&& (widthMode == MeasureSpec.EXACTLY || widthMode == MeasureSpec.AT_MOST)) {
			if (childCount > 2) {
				if (childCount > 3) {
				} else {
					mMaxTabWidth = MeasureSpec.getSize(widthMeasureSpec) / 3;
				}
			} else {
	...
	private void addTab(int index, CharSequence text, int iconResId) {
		// final TabDotView tabView = new TabDotView(getContext());
	...
	public void setTextSize(int sizeSP) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (view instanceof TextView) {
				Log.i(VIEW_LOG_TAG, "设置标题字体大小" + sizeSP);
	...
	private class TabView extends TextView {
		...
		public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			...
			if (mMaxTabWidth > 0
					&& (getMeasuredWidth() > mMaxTabWidth || mItemCount <= 4)) {
	...
	// private class TabDotView extends RelativeLayout {
	//
	// private final TextView mTextView;
	//
	// private final ImageView mDotIV;
	//
	// private int mIndex;
	//
	// public TabDotView(Context context) {
	// super(context);
	// View view = LayoutInflater.from(context).inflate(R.layout.tab_view,
	// this);
	// mTextView = (TextView) view.findViewById(R.id.text);
	// mDotIV = (ImageView) view.findViewById(R.id.dot_iv);
	// }
	//
	...
	// mDotIV.setVisibility(visibility);
	// }
	//
	// @Override
	// public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
	...
	// mTextView.setCompoundDrawablesWithIntrinsicBounds(left, top, right,
	// bottom);
	// }
	//
	// public int getIndex() {
