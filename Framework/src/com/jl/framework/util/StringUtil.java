...
public class StringUtil {
     ...
     * @return boolean
     * @throws
     * @Method: isTelNumAvailable
     */
    public static boolean isTelNumAvailable(String telNum) {
        boolean result = false;
        if (!isNullOrEmpty(telNum) && telNum.matches("^1[34578]{1}[0-9]{9}$")) {
            result = true;
        }
        return result;
    }
    ...
    /**
     * 格式化电话号码(电话号码不合法，返回原始字符串)
     *
     * @param telNum
     * @return
     */
    public static String formatTelNumStr(String telNum) {
        if (telNum != null) {
            telNum.replace(" ", "");
        } else {
            return null;
        ...
        StringBuffer stringBuffer = new StringBuffer();
        if (telNum.length() == 11) {
            stringBuffer.append(telNum.substring(0, 3)).append(" ")//
                    .append(telNum.substring(3, 7)).append(" ")//
